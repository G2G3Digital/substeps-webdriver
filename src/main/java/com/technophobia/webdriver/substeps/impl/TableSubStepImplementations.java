...
public class TableSubStepImplementations extends AbstractWebDriverSubStepImplementations{
    ...
    /* TODO
     * 
     * a method that takes a table with column names - stash in the context for reference and look up the index
     * could combine this - not sure what purpose the column names serves...
     * see nhs proxy user feature (low)
     * and Then the current contracts list will contain the following
     * ContractSearchSteps.thenTheContractsListContains
     * 
     * Assert table contains row - a method that takes a map of values, use the stash for the index
     * 
    ...
    */
    ...
    /**
     * Locates the table body row, assuming that the table has already been located
     * Row 1 is the first <tr> beneath a <tbody>
     * 
...
// TODO can't implement this until worked out how to pass single rows through
    
//    @Step("AssertTable row ([^\"]*) contains values")
//    public void assertTableRowContainsValues(@StepParameter(converter = IntegerConverter.class)final Integer row, 
//    		final List<Map<String, String>> expectedResults){
//    	
//    	Assert.assertNotNull("expecting a table of results to be specified", expectedResults);
//    	Assert.assertThat("only expecting 1 row in table data", expectedResults.size(), is(1));
//    	//
//    	
//    	final WebElement tableRow = findTableBodyRow(row);
//    	
//    	
//    	 final List<WebElement> columnElements = tableRow.findElements(By.tagName("td"));
//
...
//    }
     ...
     * Row 0 is the first <tr> beneath a <tbody>
     * 
    ...
    public String getValueInResultsTable(final WebElement tbody, final int col, final int row) {
        ...
        Assert.assertTrue("expecting more than " + col + " columns in the table, got: "
                + columnElements.size(), columnElements.size() >= col);
